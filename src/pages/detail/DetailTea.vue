            ...
            <div class="tabber tabberlive">
                <ul class="tabbernav">
                    ...
                    <li :class="{ 'tabberactive': currentActiveIndex === 2 }" @click="setChildTeaActiveClass(2)"><a
                            title="解锁对话/高默契值配方" data-hash="解锁对话/高默契值配方" style="cursor: pointer;">解锁对话/高默契值配方</a></li>
                    <wbr>
                </ul>
                ...
                <div :class="{ 'tabbertab': true, 'display-block': currentActiveIndex === 2, 'display-none': currentActiveIndex !== 2 }"
                    ...
                    <p>触发对话需要基础配方默契值大于330（无加成状态下）。
                        <br>满加成计算公式：
                        <br>实际默契值 = 基础值 × (1 + 茶室家具4000舒适度加成(12%)) × (1 + 穹顶默契无间9级加成(9%) + 真心联礼加成(10%))
                        <br>触发对话不受休息室的舒适度和白荆穹顶提供的默契值加成影响。此处仅提供不同等级时默契值最高的配方以供参考。<br>
                    </p>
                    <table class="wikitable" style="width:100%;margin:0;text-align:center;">
                        <tbody>
                            <tr>
                                ...
                                <th colspan="4" style="width:41%;">参考默契值</th>
                            </tr>
                            <tr>
                                <th style="width:5%;">基础</th>
                                <th style="width:5%;">满家具</th>
                                <th style="width:5%;">满穹顶</th>
                            ...
                            <tr v-for="(rapport, index) in allCharTea[$route.params.name]['hignRapport']" :key="index">
                                ...
                                <td v-for="(ex, index) in exInit(rapport.ex[0])" :key="index">{{ ex }}</td>
                            </tr>
                        </tbody>
                    </table>
                    <p class="mw-empty-elt"></p>
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            rapportFormula: window.$commonUtil.rapportFormula
        }
    },
    methods: {
        ...
        exInit(baseRap) {
            let tempEx = new Array
            tempEx.push(baseRap)//第一个为基础值
            for (let formulaIndex in this.rapportFormula) {
                tempEx.push(Math.round(baseRap * this.rapportFormula[formulaIndex]))
            }
            return tempEx
        }
    }
