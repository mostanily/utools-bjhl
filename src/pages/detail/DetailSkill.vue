        ...
        <div class="tabber tabberlive">
            ...
            <div v-for="(skill, index) in allSkill[$route.params.name]" :key="index"
                ...
                <div v-if="skill.aliasNum === '技能同调'">
                    <div style="color:#717880;">
                        <div style="background:#9FA9AF;padding:3px 10px;border-radius:5px;color:#FFF;">解锁条件
                        </div>
                        <hr style="margin:5px 0;background:#9FA9AF;">
                        <div>
                            <div style="padding: 3px;line-height: 32px;font-size: 16px;">获取方式：拥有烙痕【
                                <span>
                                    ...
                                    <img :alt="rarityEnName[getLaohenDetail(skill.unlock).rarity] + '.png'"
                                        :src="getLaohenRarityImg(getLaohenDetail(skill.unlock).rarity, 1)" decoding="async" loading="lazy"
                                        width="20" height="20"
                                    ...
                                    <router-link style="color:#646cff;font-size:18px;" :to="{ name: 'laohen_detail', params: { name: skill.unlock } }"
                        ...
                        <div style="background:#9FA9AF;padding:3px 10px;border-radius:5px;color:#FFF;">烙痕【{{ getLaohenDetail(skill.unlock).name }}】特质强化
                        </div>
                        <hr style="margin:5px 0;background:#9FA9AF;">
                        ...
                        <div style="background:#9FA9AF;padding:3px 10px;border-radius:5px;color:#FFF;">
                            技能{{ allSkill[$route.params.name][skill.repSkill].aliasNum }}
                            【{{ allSkill[$route.params.name][skill.repSkill].name }}】→
                            【{{ skill.detail.name }}】
                        </div>
                        <hr style="margin:5px 0;background:#9FA9AF;">
...
export default defineComponent({
    components: {
        SkillContent, LaohenSkillResourse
    },
    data() {
        return {
